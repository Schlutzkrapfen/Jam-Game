...
func _physics_process(_delta: float) -> void: ##movment and animations 
	...
	if direction.x > Mouse_pos.x and roll:##charackter dirktion
		if death :
			$Sprite.scale.x = -1
		...
		if direction.x < 0: 
			...
			if walkdirection == false:
				changedirection = true
			walkdirection = true
	elif direction.x < Mouse_pos.x  and roll:
		if death:
			$Sprite2.scale.x = 1
			$Sprite.scale.x = 1
			$Sprite.position = Vector2(8,0)
	...
	if punch == false and damagetaken:
		if direction.x == 0 and movement == false and roll:
	...
	if not roll:
		if direction.x == 0:
			direction.x = $Sprite.scale.x
		if Input.get_action_strength("Move_right")>0 and Input.get_action_strength("Move_left")<0:
			direction.x = (Input.get_action_strength("Move_right") - Input.get_action_strength("Move_left")/Input.get_action_strength("Move_right") - Input.get_action_strength("Move_left"))
...
func _input(_event):
	if Input.is_action_just_pressed("roll") and roll:
		...
		movmentspeed = 2
		yield (get_tree().create_timer(0.8),"timeout")# needs to be changed when the rollanimation will be changed#
		#(i hate to work with the animationtree) and isnt quit the right lenght but it will work for now
		set_collision_mask(5)
		roll = true
		state_machine.travel("Idle")
	if Input.is_action_just_pressed("Block") :
		block = true
		blockspeed = 0.4##how much slower the charakter gets
	...
	if Input.is_action_just_released("Hit"):
		if punch and punchischarged:
			...
			if punchposition == 1:
				damageposition = 1
				$AnimationPlayer.get_animation("Box High").length = damagemultiplyer /4
				state_machine.travel("Box")
				if damagemultiplyer > 0.5:
					$"woosch1 sound".play()
			if punchposition == 0 and mouseposition == 2:
				damageposition = 3
				$AnimationPlayer.get_animation("Box low").length = damagemultiplyer /4
...
func takedamage(amount,positionpunch):
	...
	if healt < 0:
		...
		return
	if blocking and roll:
		healt -=amount 
		emit_signal("camara",0.2* amount,amount/2,Type.Random)
		$Particles2D.set_amount(amount*10)
		$Particles2D.set_lifetime(amount/2)
		$Particles2D.process_material.set_param(true,amount *100)
		$DamageTimer.wait_time =amount/3
		$DamageTimer.start()
		damagetaken= false
		punchischarged = false
		if positionpunch == 0 or positionpunch ==3:
...
func _on_Punch_body_entered(body):
	...
	if body.has_method("knockback") :
		body.knockback(damagemultiplyer)
...
func greifdrone(amount,positionpunch): # when the grefdrone makes damage to you
	...
	$Particles2D.set_lifetime(amount/2)
	$Particles2D.process_material.set_param(true,amount *100)
	$DamageTimer.wait_time =amount/8
	$DamageTimer.start()
		...
		if healt < 0:
			...
			if death:
				...
				if positionpunch == 0 or positionpunch == 3:
					...
					if amount > 0.25:
						$Punch_sound.set_volume_db((amount-3.5)*4 )
						$Punch_sound.play()#evry sound must be changed to grab sounds
					if amount < 0.25:
						$Punch_sound1.set_volume_db((amount-3.5)*4 )
						$Punch_sound1.play()
					$Death1.set_volume_db((amount-3.5)*4 )
...
func _on_Level2_Healt_lvlStart(helth_lvlstart):
